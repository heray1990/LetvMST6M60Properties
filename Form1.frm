    ...
    For i = 1 To itemNumOfTvInfo
        ...
        If isCmdDataRecv = False Then
            If j > cmdResendTimes Then
                j = 0
                Log_Info "Cannot read the property."
                MsgBox "无法与电视通讯，请检查线材与板卡。"
                GoTo FAIL
            Else
                j = j + 1
                i = i - 1
...
PASS:
    ...
    If MsgBox("PASS，请拔掉线材并且点击OK", vbOKOnly, "PASS") = vbOK Then
        lbResult.Caption = "Initial"
...
FAIL:
    lbResult.Caption = "FAIL"
    lbResult.BackColor = &HFF&
    If MsgBox("FAIL，请拔掉线材并且点击OK", vbOKOnly, "FAIL") = vbOK Then
        lbResult.Caption = "Initial"
        lbResult.BackColor = &HFFFFFF
        CommandStart.Enabled = True
    End If
